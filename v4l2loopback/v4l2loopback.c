...
#define iprintk(fmt, args...)                                          \
        do {                                                           \
            if (debug >= 0) {                                       \
                printk(KERN_INFO "%s[" __stringify( \
                           __LINE__) "], pid(%d):  " fmt,  \
                           __func__, task_pid_nr(current), ##args);          \
            }                                                      \
        } while (0)
...
#define _FORCE_KEEP_ALLOC_BUFFER_ 1 //xlli add for keep alloc buffer
...
#define V4L2LOOPBACK_SIZE_DEFAULT_WIDTH 3840
#define V4L2LOOPBACK_SIZE_DEFAULT_HEIGHT 2160
...
#if (_FORCE_KEEP_ALLOC_BUFFER_ == 0)
...
static int inner_try_setfmt(struct file *file, struct v4l2_format *fmt)
{
	...
	dprintk("%s : capture(%d) needschange:%d, dev->active_readers:%d, dev->ready_for_capture:%d\n", __func__,
					capture, needschange, dev->active_readers, dev->ready_for_capture);
 ...
 */
static int loopback_vidioc_querycap(struct file *file, void *priv,
			   struct v4l2_capability *cap)
{
	...
	iprintk("dev->announce_all_caps : %d\n", dev->announce_all_caps);
...
static int loopback_vidioc_enum_framesizes(struct file *file, void *fh,
				  struct v4l2_frmsizeenum *argp)
{
	...
	if (V4L2LOOPBACK_IS_FIXED_FMT(dev)) {
		...
		iprintk("argp->discrete size [%dx%d]\n", argp->discrete.width, argp->discrete.height);
	} else {
		...
		iprintk("argp->stepwise size [%dx%d] - [%dx%d]\n",
 ...
 */
static int loopback_vidioc_enum_frameintervals(struct file *file, void *fh,
				      struct v4l2_frmivalenum *argp)
{
	...
	if (V4L2LOOPBACK_IS_FIXED_FMT(dev)) {
		...
		iprintk("argp->discrete numerator:%d, denominator:%d\n", argp->discrete.numerator, argp->discrete.denominator);
	} else {
		...
		iprintk("argp->stepwise min.denominator :%d, max.denominator :%d\n",
		        argp->stepwise.min.denominator,
 ...
 */
static int loopback_vidioc_enum_fmt_cap(struct file *file, void *fh,
 ...
 */
static int loopback_vidioc_g_fmt_cap(struct file *file, void *priv,
			    struct v4l2_format *fmt)
{
	...
	dprintk("%s : format : %d, fourcc : %s\n", __func__, fmt->fmt.pix, fourcc2str(dev->pix_format.pixelformat, buf));
 ...
 */
static int loopback_vidioc_try_fmt_cap(struct file *file, void *priv,
			      struct v4l2_format *fmt)
{
	int ret = 0;
 ...
 */
static int loopback_vidioc_s_fmt_cap(struct file *file, void *priv,
			    struct v4l2_format *fmt)
{
	int ret = 0;
	char buf[5] = {0};
...
#if (_FORCE_KEEP_ALLOC_BUFFER_ == 1)
...
#endif
	dprintk("%s : width:%d, height:%d, fourcc: %s, sizeimage : %d\n", __func__, fmt->fmt.pix.width, fmt->fmt.pix.height, fourcc2str(dev->pix_format.pixelformat, buf), fmt->fmt.pix.sizeimage);
 ...
 */
static int loopback_vidioc_enum_fmt_out(struct file *file, void *fh,
			       struct v4l2_fmtdesc *f)
{
	struct v4l2_loopback_device *dev;
	const struct v4l2l_format *fmt;
 ...
 */
static int loopback_vidioc_g_fmt_out(struct file *file, void *priv,
			    struct v4l2_format *fmt)
{
	struct v4l2_loopback_device *dev;
	char buf[5] = {0};
 ...
 */
static int loopback_vidioc_try_fmt_out(struct file *file, void *priv,
			      struct v4l2_format *fmt)
{
	int ret = 0;
 ...
 */
static int loopback_vidioc_s_fmt_out(struct file *file, void *priv,
			    struct v4l2_format *fmt)
{
	struct v4l2_loopback_device *dev;
	int ret = 0;
...
static int loopback_vidioc_g_fmt_overlay(struct file *file, void *priv,
				struct v4l2_format *fmt)
{
	return 0;
}

static int loopback_vidioc_s_fmt_overlay(struct file *file, void *priv,
				struct v4l2_format *fmt)
{
	return 0;
}
 ...
 */
static int loopback_vidioc_g_parm(struct file *file, void *priv,
 ...
 */
static int loopback_vidioc_s_parm(struct file *file, void *priv,
			 struct v4l2_streamparm *parm)
{
	struct v4l2_loopback_device *dev;
	int err = 0;
 ...
 */
static int loopback_vidioc_s_std(struct file *file, void *fh, v4l2_std_id *_std)
{
 ...
 */
static int loopback_vidioc_g_std(struct file *file, void *fh, v4l2_std_id *norm)
{
	if (norm)
		*norm = V4L2_STD_ALL;
	return 0;
 ...
 */
static int loopback_vidioc_querystd(struct file *file, void *fh, v4l2_std_id *norm)
{
	if (norm)
		*norm = V4L2_STD_ALL;
	return 0;
 ...
 */
static int loopback_vidioc_enum_output(struct file *file, void *fh,
 ...
 */
static int loopback_vidioc_g_output(struct file *file, void *fh, unsigned int *i)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	if (!dev->announce_all_caps && !dev->ready_for_output)
		return -ENOTTY;
 ...
 */
static int loopback_vidioc_s_output(struct file *file, void *fh, unsigned int i)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	if (!dev->announce_all_caps && !dev->ready_for_output)
		return -ENOTTY;
 ...
 */
static int loopback_vidioc_enum_input(struct file *file, void *fh,
 ...
 */
static int loopback_vidioc_g_input(struct file *file, void *fh, unsigned int *i)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
 ...
 */
static int loopback_vidioc_s_input(struct file *file, void *fh, unsigned int i)
{
	struct v4l2_loopback_device *dev = v4l2loopback_getdevice(file);
	if (!dev->announce_all_caps && !dev->ready_for_capture)
		return -ENOTTY;
 ...
 */
static int loopback_vidioc_reqbufs(struct file *file, void *fh,
			  struct v4l2_requestbuffers *b)
{
	...
	iprintk("memory:%d\t%d=%d, opener->timeout_image_io:%d\n", b->memory, b->count,
		dev->buffers_number, opener->timeout_image_io);
	...
	if (V4L2_TYPE_IS_OUTPUT(b->type) && (!dev->ready_for_output)) {
		dprintk("%s : output buffer not ready!\n", __func__);
		return -EBUSY;
	}
	...
	case V4L2_MEMORY_MMAP:
		...
		dprintk("%s : opener->buffers_number : %d, dev->used_buffers : %d\n", 
		    __func__, opener->buffers_number, dev->used_buffers);
 ...
 */
static int loopback_vidioc_querybuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
 ...
 */
static int loopback_vidioc_qbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	struct v4l2_loopback_device *dev;
	struct v4l2_loopback_opener *opener;
...
static int get_capture_buffer(struct file *file)
{
	...
	if (timeout_happened) {
		if (ret < 0) {
			iprintk("trying to return not mapped buf[%d]\n", ret);
			return -EFAULT;
		}
 ...
 */
static int loopback_vidioc_dqbuf(struct file *file, void *fh, struct v4l2_buffer *buf)
{
	struct v4l2_loopback_device *dev;
	struct v4l2_loopback_opener *opener;
	int index = 0;
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		*buf = dev->buffers[index].buffer;
		buf->sequence = (__u32)(opener->read_position - 1);
		dprintkrw(
 ...
 */
static int loopback_vidioc_streamon(struct file *file, void *fh, enum v4l2_buf_type type)
{
	struct v4l2_loopback_device *dev;
	struct v4l2_loopback_opener *opener;
	MARK();
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		return 0;
	default:
		iprintk("Invalid v4l2 buffer type: %d\n", type);
		return -EINVAL;
	}
	return -EINVAL;
}
 ...
 */
static int loopback_vidioc_streamoff(struct file *file, void *fh,
			    enum v4l2_buf_type type)
{
	struct v4l2_loopback_device *dev;
	struct v4l2_loopback_opener *opener;
...
static int loopback_vidiocgmbuf(struct file *file, void *fh, struct video_mbuf *p)
{
	struct v4l2_loopback_device *dev;
	MARK();
...
static int loopback_vidioc_subscribe_event(struct v4l2_fh *fh,
				  ...
				  size_t count, loff_t *ppos)
{
	...
	dprintkrw("leave v4l2_loopback_read %zu bytes\n",count);
	return count;
}
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (UNNEGOTIATED == opener->type) {
		...
		if (dev->ready_for_output) {
			err = loopback_vidioc_streamon(file, file->private_data,
		...
		if (err < 0) {
			iprintk("loopback_vidioc_streamon err : %d\n", err);
			return err;
		}
}

	if (WRITER != opener->type) {
		iprintk("opener->type err : %d\n", opener->type);
		return -EINVAL;
	}
...
static void try_free_buffers(struct v4l2_loopback_device *dev)
{
	iprintk("counter:%d, keep_format:%d\n", dev->open_count.counter, dev->keep_format);
...
static void init_buffers(struct v4l2_loopback_device *dev)
{
	...
	iprintk(" size:%lu, bytesused:%d, buffers_number:%d\n", buffer_size, bytesused, dev->buffers_number);
	for (i = 0; i < dev->buffers_number; ++i) {
...
static int v4l2_loopback_add(struct v4l2_loopback_config *conf, int *ret_nr)
{
	...
	int _pixelformat = formats[0].fourcc;
	...
	if (err)
		goto out_unregister;
	dev->keep_format = _FORCE_KEEP_ALLOC_BUFFER_;
	...
	dev->pix_format.pixelformat = _pixelformat;
	dev->pix_format.colorspace = V4L2_COLORSPACE_DEFAULT; /* do we need to set this ? */
	dev->pix_format.field = V4L2_FIELD_NONE;
#if (_FORCE_KEEP_ALLOC_BUFFER_ == 1)
	dev->pix_format.sizeimage = _width * _height * 2; //xlli Add for keep alloc buffer
#endif
	dev->buffer_size = PAGE_ALIGN(dev->pix_format.sizeimage);
	iprintk("width : %d, height : %d, buffer_size = %ld (sizeimage=%d) format '%c%c%c%c'\n", 
				dev->pix_format.width,
				dev->pix_format.height,
				dev->buffer_size,
				dev->pix_format.sizeimage,
				(_pixelformat >> 0) & 0xFF, (_pixelformat >> 8) & 0xFF,
				(_pixelformat >> 16) & 0xFF, (_pixelformat >> 24) & 0xFF);
...
static const struct v4l2_ioctl_ops v4l2_loopback_ioctl_ops = {
	// clang-format off
	.vidioc_querycap		= &loopback_vidioc_querycap,
	.vidioc_enum_framesizes		= &loopback_vidioc_enum_framesizes,
	.vidioc_enum_frameintervals	= &loopback_vidioc_enum_frameintervals,

	.vidioc_enum_output		= &loopback_vidioc_enum_output,
	.vidioc_g_output		= &loopback_vidioc_g_output,
	.vidioc_s_output		= &loopback_vidioc_s_output,

	.vidioc_enum_input		= &loopback_vidioc_enum_input,
	.vidioc_g_input			= &loopback_vidioc_g_input,
	.vidioc_s_input			= &loopback_vidioc_s_input,

	.vidioc_enum_fmt_vid_cap	= &loopback_vidioc_enum_fmt_cap,
	.vidioc_g_fmt_vid_cap		= &loopback_vidioc_g_fmt_cap,
	.vidioc_s_fmt_vid_cap		= &loopback_vidioc_s_fmt_cap,
	.vidioc_try_fmt_vid_cap		= &loopback_vidioc_try_fmt_cap,

	.vidioc_enum_fmt_vid_out	= &loopback_vidioc_enum_fmt_out,
	.vidioc_s_fmt_vid_out		= &loopback_vidioc_s_fmt_out,
	.vidioc_g_fmt_vid_out		= &loopback_vidioc_g_fmt_out,
	.vidioc_try_fmt_vid_out		= &loopback_vidioc_try_fmt_out,

#ifdef V4L2L_OVERLAY
	.vidioc_s_fmt_vid_overlay	= &loopback_vidioc_s_fmt_overlay,
	.vidioc_g_fmt_vid_overlay	= &loopback_vidioc_g_fmt_overlay,
#endif

#ifdef V4L2LOOPBACK_WITH_STD
	.vidioc_s_std			= &loopback_vidioc_s_std,
	.vidioc_g_std			= &loopback_vidioc_g_std,
	.vidioc_querystd		= &loopback_vidioc_querystd,
#endif /* V4L2LOOPBACK_WITH_STD */

	.vidioc_g_parm			= &loopback_vidioc_g_parm,
	.vidioc_s_parm			= &loopback_vidioc_s_parm,

	.vidioc_reqbufs			= &loopback_vidioc_reqbufs,
	.vidioc_querybuf		= &loopback_vidioc_querybuf,
	.vidioc_qbuf			= &loopback_vidioc_qbuf,
	.vidioc_dqbuf			= &loopback_vidioc_dqbuf,

	.vidioc_streamon		= &loopback_vidioc_streamon,
	.vidioc_streamoff		= &loopback_vidioc_streamoff,

#ifdef CONFIG_VIDEO_V4L1_COMPAT
	.vidiocgmbuf			= &loopback_vidiocgmbuf,
#endif

	.vidioc_subscribe_event		= &loopback_vidioc_subscribe_event,
